...
{#each page.visibleData as artist, i}
  ...
  <div class="pt-2 pb-4 flex flex-col">
    {#if !page.currentHandle}
      <h1 class="transition duration-300 ease-linear text-lg font-bold text-gray-700 hover:text-indigo-500"><a href="/taiteilijat/{handleize(artist.nimi)}" title="Taiteilijan {artist.nimi} koostesivu">{artist.nimi}</a></h1>
      <div class="flex flex-row items-center justify-between">
        ...
        {#if artist.stamps.length > limit}
          <a href="/taiteilijat/{handleize(artist.nimi)}" title="Taiteilijan {artist.nimi} koostesivu" class="transition duration-300 ease-linear py-1 px-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-100 flex flex-row items-center hover:bg-indigo-200 text-indigo-500 bg-indigo-50">
  ...
  <div class="grid { !page.currentHandle ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6 pb-8 lg:pb-12' : 'grid-cols-1 md:grid-cols-3 lg:grid-cols-4 pb-4  '} grid-flow-row gap-0">
    {#if !page.currentHandle}
      {#each artist.stamps.slice(0,limit) as stamp}
