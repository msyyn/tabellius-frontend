  ...
  <div class="grid { !page.currentHandle ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6 pb-8 lg:pb-12' : 'grid-cols-1 md:grid-cols-3 lg:grid-cols-4 pb-4  '} grid-flow-row gap-0">
    {#if !page.currentHandle}
      {#each artist.stamps.slice(0,limit) as stamp}
        <a rel="prefetch" in:fade={{delay: i * 40}} class="transition duration-300 ease-linear border items-center border-indigo-50 p-2 flex flex-col bg-white -mr-0.5 -mb-0.5 hover:z-10 hover:shadow-inner hover:border-indigo-100" href="postimerkit/{handleize(stamp.merkin_nimi)}{stamp.merkin_vari.length ? `?versio=${generateHandle(stamp)}` : ''}">
    ...
    {:else}
      {#each artist.stamps as stamp}
        <a rel="prefetch" in:fade={{delay: i * 40}} class="transition duration-300 ease-linear border items-center border-indigo-50 p-2 flex flex-row bg-white -mr-0.5 -mb-0.5 hover:z-10 hover:shadow-inner hover:border-indigo-100" href="postimerkit/{handleize(stamp.merkin_nimi)}{stamp.merkin_vari.length ? `?versio=${generateHandle(stamp)}` : ''}">
